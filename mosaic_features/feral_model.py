...
classification.  Training consumes interaction crop videos + per-frame
labels, producing per-frame behavior predictions.

This module wraps the FERAL training loop and inference to work within
Mosaic's ``Dataset.train_model()`` / ``predict_sequence()`` protocol.
"""
...
class FERALModel:
    ...
    Follows the Mosaic model protocol:
      ...
      - ``train()``: run training loop, return metrics dict
    ...
    -----------
    video_dir : str
        Directory containing interaction crop videos.
    label_json : str
        Path to FERAL-format label JSON (see feral_label_converter.py).
    feral_code_dir : str
        Path to the FERAL code directory (containing model.py, dataset.py, etc.).
    model_name : str
    ...
    freeze_encoder_layers : int
        Number of ViT layers to freeze from bottom (default 12).
    class_weights : str
        ...
        for key in required:
            if key not in cfg:
                raise ValueError(
                    ...
                    f"Provide the path to the FERAL code, video directory, "
                    f"and label JSON."
                )
    ...
    def train(self, progress_callback=None) -> dict:
        ...
        # Add FERAL code to path
        feral_dir = cfg["feral_code_dir"]
        ...
        from utils import prep_for_answers, save_model
        from metrics import (
    ...
    def load_trained_model(self, run_root: Path):
        ...
        model = HFModel(
            ...
            freeze_encoder_layers=cfg.get("freeze_encoder_layers", 12),
        )
    ...
    def predict_sequence(self, df_feat: pd.DataFrame, meta: dict) -> pd.DataFrame:
        """Run inference on interaction crop videos for one sequence.

        Parameters
        ----------
        df_feat : pd.DataFrame
            Metadata from InteractionCropPipeline with ``video_path`` column.
        meta : dict
        ...
        pd.DataFrame
            ...
            predicted_class, trophallaxis_prob
        """
        import torch
        ...
        from dataset import get_frame_ids, read_range_video_decord, get_frame_count
        import torchvision
