...
Identifies frames where pairs of individuals meet distance and angular
thresholds (e.g. trophallaxis in honeybees: bees facing each other within
a certain distance). Applies morphological filtering to remove noise and
enforces a minimum interaction duration.

Ported from the Konstanz trophallaxis video generation notebook
(trophollaxis_behavior_detection/video_processing/
 Trophallaxis_video_generation_Konstanz_data.ipynb).
"""
...
from mosaic.behavior.feature_library.helpers import ensure_columns
...
def _check_interaction_conditions(
    ...
    # Check if individuals face roughly *opposite* directions (mouth-to-mouth)
    inv_orient_diff = np.pi - orient_diff
    angle_ok = inv_orient_diff < np.deg2rad(max_inv_orientation_diff_deg)

    return angle_ok & (dist < max_dist)
...
class PairInteractionFilter:
    ...
    distance and angular criteria, applies morphological filtering, and
    extracts continuous interaction segments that meet a minimum duration.
